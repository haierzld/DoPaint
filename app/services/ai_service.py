    ...
    def generate_video(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"任务提交失败: {response.status_code} - {response.message}"
                )
                return None
    ...
    def poll_task(task_id: str, max_wait: int = 180) -> dict | None:
        ...
        try:
            ...
            while True:
                ...
                if response.status_code != 200:
                    logger.warning(
                        f"查询任务失败: {response.status_code} - {response.message}"
                    )
                ...
                if status == "SUCCEEDED":
                    ...
                    msg = getattr(response.output, "message", "任务失败")
